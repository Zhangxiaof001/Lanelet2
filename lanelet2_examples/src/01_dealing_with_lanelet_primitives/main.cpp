...
void part0Primitives() {
  ...
  Point3d pCopy = p;  // 注意这里相当于共享地址，任一个对象修改，另一个对象也同样修改
  assert(p == pCopy);

  // p is mutable, meaning we can change everything we passed at construction time.
  // 访问x、y、z、id，注意只有id()函数返回的是不可修改的值
  const double p_x = p.x();
  const double p_y = p.y();
  const double p_z = p.z();
  const Id p_id = p.id();
  std::cout << "Get p_x : " << p_x << " p_y : " << p_y << " p_z : " << p_z << " p_id : " << p_id << std::endl;

  double test_x = p.x();
  test_x = 100;
  std::cout << "test_x : " << test_x << " p.x() : " << p.x() << std::endl;
  ...
  p.z() = 2;
  p.setId(utils::getId());
  std::cout << "Fixed p_x : " << p.x() << " p_y : " << p.y() << " p_z : " << p.z() << " p_id : " << p.id() << std::endl;
  ...
  assert(p.z() == pCopy.z());
  std::cout << "Fixed pCopy x : " << pCopy.x() << " y : " << pCopy.y() << " z : " << pCopy.z() << " id : " << pCopy.id() << std::endl;
  ...
  // 修改或新增属性的方式
  p.attributes()["type"] = "point3d"; // 没有该属性以此方式新增
  p.attributes()["type"] = "point"; // 已有该属性以此方式修改
  ...
  // 访问属性
  auto type1 = p.attribute("type");
  auto type2 = p.attributes()["type"];
...
void part1Points() {
  ...
  assert(pTwice.z() == 8);
  std::cout << "p3d.z : " << p3d.z() << " pTwice.z : " << pTwice.z() << std::endl;
  ...
  // utils::to2D适用于各种元素
  Point2d p2d = utils::to2D(p3d);  // 相当于拷贝指针
